...
class CACHE : public MEMORY {
  public:
    ...
    uint8_t cache_type;
   uint64_t delay[DRAM_CHANNELS][DRAM_RANKS][DRAM_BANKS]; //code modification -chirayu
...
/*	FILE *fptr;
	//fptr = fopen("/home/chirayu/Downloads/champSim_delay_variation/process_config/delay.cfg","r");
	//Code modification
    	uint64_t delay[DRAM_CHANNELS][DRAM_RANKS][DRAM_BANKS];
	int del;
	...
	total_miss_latency = 0;
        ...
        pf_useless = 0;
        pf_fill = 0;
        writeback_pf_useful=0;
        fillcache_pf_useful=0;
    };
	...
	//Chirayu code modification
    uint64_t delay_increment[DRAM_CHANNELS][DRAM_RANKS][DRAM_BANKS];
   ...
   uint32_t dram_get_channel (uint64_t address); 
    uint32_t read_bank;
    uint32_t read_rank;
    uint32_t read_channel;
   ///////////////////////////////////////////////////
    int get_metadata(uint64_t phy_addr);
    int write_metadata(uint64_t phy_addr);
    void complete_metadata_req(uint64_t phy_addr);
    void aux_prefetch(uint64_t base_addr, uint64_t ip);
    void attempt_prefetch(uint64_t base_addr, int base_block, int PT_idx, int la_candidate, int pf_candidate,uint64_t ip);
    void lookahead(uint64_t base_addr, int base_block, int prev_conf, int PT_idx, int la_candidate, uint64_t ip);
